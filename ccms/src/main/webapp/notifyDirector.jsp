...
try{
	  DBHelper.log(null, strCurLogin, "notifyDirector.jsp?n=" + strLog);
	 if (strLog.length() > 0) {
	    ...
	    */
        ...
        String strSql = "select * from tblOwnerAllSignature where logID=" + strLog;
        rs = st.executeQuery(strSql);
        while (rs.next()){
        ...
        if (strApproverEmails.length() > 0){
              strApproverEmails = strApproverEmails.substring(1);
               ...
               strMsg += "Change Request: "+strLog+" has been implemented and all required followup completed successfully<br/>";
               strMsg += " Click <a href='#SERVER#/ccms/reviewRequest3.jsp?n=" + strLog + "' target='review'>here</a> to review the request.";
   
               MailSender.sendEMail(strApproverEmails, "Change Request:" + strLog + " is implemented", strMsg);
               String logMsg = "notifyDirector.jsp:" + strLog + " successfully to " + strApproverEmails;
    ...
    out.println("success");
     	   ...
     	   DBHelper.log(null,strCurLogin,"notifyDirector.jsp:"+e.toString());
	       out.println("Error:"+e);
   }
  finally{
