...
<%
  ...
  String strEmergencyCondition=" 1=1";
  ...
  if (strStatusName.toUpperCase().equals("OVERDUE")){
	   strStatusCondition=" (log.ApprovalStatusID="+strStatus+" or (log.ApprovalStatusID in (select approvalstatusid from tblApprovalStatus where approvalStatus='Approved') and log.endtiming<dateadd(day,-1,getdate()) ))";
  }
...
<form>
<table>
<tr><th>Log#</th><th>RequestDate</th><th>Originator</th><%-- <th>Team</th> --%><th>Department</th><th>Line</th><th>Equipment</th><th>Start of Construction</th><th>Project Startup</th> <%-- <th>costType</th><th>Cost</th>  --%> <th>Status</th><th>EO/Chemical Clearance?</th><th>Emergency?</th>
</tr>
<%
  Connection c=null;
  Statement st=null;
  ...
  ResultSet rs =null;
  ResultSet rs2=null;
  String sql="select log.*,appr.*,e.equipmentName, isnull(CONVERT(VARCHAR(10), log.requestDate, 101),'') requestDateStr,isnull(CONVERT(VARCHAR(10), log.startTiming, 101),'') startTimingStr,isnull(CONVERT(VARCHAR(10), log.endTiming, 101),'') endTimingStr   from tblChangeControlLog log,tblApprovalStatus appr, tblEquipment e where (log.hidden is null or log.hidden<>'Y') and log.approvalstatusid=appr.approvalstatusid and log.equipmentID=e.equipmentID";
  sql+= " and "+strOriginatorCondition;
  sql+= " and "+strCreatorTeamCondition;
  sql+= " and "+strEmergencyCondition;
  sql+= " and "+strSafetyCondition;
...
//  out.println(sql);
  ...
  try {
    ...
    while (rs.next())  {
		...
		String strCost2_t=rs.getString("cost2");
		...
		%>
        <tr class="<%=strClass%>">
		 <td><a href="./reviewRequest3.jsp?n=<%=iLogID_t%>"><%=iLogID_t%></a></td><td><%=strRequestDate_t%></td>
		 <td><%=strOriginator_t%></td><%-- <td><%=strCreatorTeam_t%></td> --%> <td><%=strAreaNames%></td><td><%=strLineNames%></td><td><%=strEquipment_t%></td>
		 <td><%=strStartTiming_t%></td><td><%=strEndTiming_t%></td> <%-- <td><%=strCostType_t%></td><td><%=strCost2_t%></td> --%> <td><%=strStatus_t%></td><td><%=strSafety%></td><td><%=strEmergency%></td>
