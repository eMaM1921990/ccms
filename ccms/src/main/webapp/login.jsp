...
<%@ page
	import="javax.naming.*, javax.naming.directory.*, java.util.Hashtable"%>

<%@page import="com.pg.ccms.utils.DBHelper"%>
<%@page import="java.sql.*"%>

<%!boolean ldapAuthenticate(String ldapUsername, String ldapPwd)
			throws Exception {
		if (ldapUsername.equals("test1"))
			return true;
		DirContext ctx = null;
		...
		try {
			...
			env.put(Context.PROVIDER_URL,
					"ldap://peoplefinder.internal.pg.com:389/");//using anonymous search
			//env.put(Context.PROVIDER_URL, "ldap://dc-01:389/");
			ctx = new InitialDirContext(env);

			answer = ctx.search("ou=people, ou=pg, o=World", matchAttrs,
					attrIDs);
			if (answer.hasMore()) {
		...
		env.put(Context.INITIAL_CONTEXT_FACTORY,
				"com.sun.jndi.ldap.LdapCtxFactory");
		env.put(Context.PROVIDER_URL,
				"ldap://peoplefinder.internal.pg.com:989/");
		try {
			env.put(Context.INITIAL_CONTEXT_FACTORY,
					"com.sun.jndi.ldap.LdapCtxFactory");
			env.put(Context.PROVIDER_URL,
					"ldap://peoplefinder.internal.pg.com:636/");
			env.put(Context.SECURITY_AUTHENTICATION, "simple");
			env.put(Context.SECURITY_PROTOCOL, "ssl");

			env.put(Context.SECURITY_PRINCIPAL, "uid=" + userTnum
	...
	String getLDAPAttribute(String user, String attr) throws Exception {
		...
		try {
			...
			env.put(Context.INITIAL_CONTEXT_FACTORY,
					"com.sun.jndi.ldap.LdapCtxFactory");
			env.put(Context.PROVIDER_URL,
					"ldap://peoplefinder.internal.pg.com:389");
			ctx1 = new InitialDirContext(env);

			answer = ctx1.search("ou=people, ou=pg, o=World", matchAttrs,
					attrIDs);
			if (answer.hasMore()) {
				SearchResult sr = (SearchResult) answer.next();
				Attributes attrs = sr.getAttributes();
	...
	String isAreaOwner(String user) throws Exception {
		...
		ResultSet rs = null;
		String sql = "select o.areaid from tblAreaOwners o,tblArea a where o.areaid=a.areaid and (a.hidden<>'Y' or a.hidden is null) and charindex(';"
				+ user + ";',';'+changeOwnerAccount+';')>0";
	...
	boolean isApprover(String user) throws Exception {
		...
		ResultSet rs = null;
		String sql = "select distinct approverAccout from tblApprovers where (hidden<>'Y' or hidden is null) and charindex(';"
				+ user + ";',';'+approverAccout+';')>0";
		...
		try {
			...
			if (rs.next())
				...
				sql = "select * from tblLine l where (l.hidden<>'Y' or l.hidden is null) and charindex(';"
						+ user + ";',';'+owneraccount+';')>0";
				rs = st.executeQuery(sql);
				if (rs.next())
					bIsApprover = true;
				else {
					sql = "select * from tblEquipmentOwner eo where (eo.hidden<>'Y' or eo.hidden is null) and charindex(';"
							+ user + ";',';'+owneraccount+';')>0";
					rs = st.executeQuery(sql);
					if (rs.next())
						bIsApprover = true;
	...
	boolean isDirector(String user) throws Exception {
		...
		ResultSet rs = null;
		String sql = "SELECT * FROM tblCMSUser WHERE director = 'Y' AND account = '"
				+ user + "'";
	...
	String getUserType2(String u, String email, String fromSite, String fullName)
			throws Exception {
		if (u.equals("test1"))
			return "SYSADMIN";
		Connection c = null;
		...
		try {
			...
			if (rs.next()) {
				...
				pstmt.setString(
						6,
						fromSite.toUpperCase().indexOf(
								siteParameter.toUpperCase()) > -1 ? "Y" : "N");
			...
			rs = st.executeQuery(sql);
			if (rs.next()) {
				String strActiveUser = rs.getString("active") == null
						? "N"
						: rs.getString("active");
				String strFromSite = rs.getString("site") == null ? "" : rs
						.getString("site");
				strReturn = rs.getString("type") == null ? "user" : rs
						.getString("type");
				String strAllowAccess = rs.getString("allowAccess") == null
						? "Y"
						: rs.getString("allowAccess");

				if (!strActiveUser.trim().toUpperCase().equals("Y"))
					throw new Exception("Inactive user");
				if (siteParameter.length() > 0
						&& strFromSite.toUpperCase().indexOf(
								siteParameter.toUpperCase()) < 0) {
					if (!strAllowAccess.equals("Y"))
						throw new Exception("You(" + u
								+ ") are not from Site: " + siteParameter
...
<%
	...
	strUser = request.getParameter("user") == null ? "" : request
			.getParameter("user");
	strPwd = request.getParameter("pwd") == null ? "" : request
			.getParameter("pwd");

	strUser = strUser.trim().toLowerCase();
	// strPwd=strPwd.trim();
	String strRedirectUrl = request.getParameter("url") == null
			? ""
	...
	if (bLogin) {
		...
		try {
			if (strUser.equals("test1")) {
				...
				strSite = getLDAPAttribute(strUser, "ExtSite");
			}
			if (strSite == null) {
				String strSite2 = getLDAPAttribute(strUser,
						"ExtDepartmentName");
				strSite = strSite2;
			}
			...
			String delegatedIDs = DelegateHelper
					.getDelegatedOwnerIDs(strUser);
			...
			if (isDirector(strUser)
					|| DelegateHelper.isDirectorDelegate(strUser))
			...
			session.setAttribute(
					"usertype",
					getUserType2(strUser, strEmail, strSite,
							strFullName));
			DBHelper.log(null, strUser, "login.jsp:" + strRemoteAddr
					+ " login sucessfully");
		} catch (Exception ee) {
		...
		if (strRedirectUrl.toLowerCase().trim().indexOf("header.jsp") > -1
				|| strRedirectUrl.toLowerCase().trim()
						.indexOf("toc.jsp") > -1)
			bValidUrl = false;
		if (strRedirectUrl.toLowerCase().trim().indexOf("ccms.jsp") > -1
				|| strRedirectUrl.toLowerCase().trim()
						.indexOf("login.jsp") > -1)
			bValidUrl = false;
	...
	if (bLogin && !bError) {
		...
		DBHelper.log(null, strUser, "Error-login.jsp:" + strRemoteAddr
				+ " " + strErr);
	}
	if (!bLogin || bError) {
%>
...
<body topmargin="120" onLoad="document.getElementById('user').focus();"
	style="font-family: arial">
	<form name="the_form" id="the_form" method="post" action="#"
		onSubmit="return check();">

		<table align='center'
			style="border-style: solid; border-width: 1; border-color: #B0B0B0; border-collapse: collapse; padding-left: 4; padding-right: 4; padding-top: 1; padding-bottom: 1"
			class="content_body">

			<tr>
				<td colspan='2'><img src='./images/logo1.jpg'></td>
			</tr>
			<tr>
				<td height="50" colspan='2'
					style="vertical-align: top; text-align: center;">Please
					provide your login information to access the system</td>
			</tr>
			<tr>
				<td width="150px" align='right'>Intranet User Name:<br />(example:
					doe.j)
				</td>
				<td width="300px" align="left"><input class="login"
					style="font-size: 14pt; font-weight: bold; font-family: arial;"
					type="text" name='user' id='user' value='<%=strUser%>' size='22'></td>
			</tr>
			<tr>
				<td width="150px" align='right'>Intranet Password:</td>
				<td width="300px" align="left"><input class="password"
					type="password" name='pwd' id='pwd' value='<%=strPwd%>' size='22'></td>
			</tr>
			<%
				if (bError) {
			%>
			<tr style="background-color: #E0E0E0">
				<td colspan='2' align='left'><div
						style="font-size: 10pt; color: #F00; font-weight: bold;"><%=strErr%></div></td>
			</tr>
			<%
				}
			%>
			<tr>
				<td colspan="2" style="text-align: center;"><br /> <input
					type='submit' name='btnSubmit' value='Login'></td>
			</tr>
			<tr class="content_footer">
				<td colspan="2" align="right" style="font-size: 8pt;">&copy;
					2011 P&amp;G <span style="margin-left: 30px;"><a
						onClick="window.open('about.jsp','aboutWin','menubar=0,resizable=0,width=500,height=350,toolbar=0');return false;"
						href="void(0)">About</a></span>
				</td>
			</tr>
